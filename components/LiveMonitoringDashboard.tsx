...
import { Activity, User, Clock, Square, ArrowUpRight, RefreshCw, CheckCircle2 } from "lucide-react";
...
export function LiveMonitoringDashboard() {
  ...
  const handleAdminStopSession = async (sessionId: string) => {
    try {
      setStoppingId(sessionId);
      const res = await fetch("/api/sessions/stop", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sessionId, note: "Stopped by admin" }),
      });
      if (res.ok) {
                ...
                <CardContent className="pt-0 text-xs text-muted-foreground space-y-3">
                  <div className="flex items-center justify-between bg-zinc-100/70 dark:bg-zinc-800/50 p-2.5 rounded-lg">
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => handleAdminStopSession(session._id)}
                      disabled={stoppingId === session._id}
                      className="cursor-pointer h-7 text-[11px] px-2.5 gap-1"
                    >
                      <Square className="w-3 h-3 fill-current" />
                      {stoppingId === session._id ? "Stopping..." : "Admin Stop"}
                    </Button>
                  </div>
                </CardContent>
              </Card>
